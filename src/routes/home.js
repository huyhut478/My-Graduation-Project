...
router.get('/', async (req, res) => {
  try {
    ...
    switch (sort) {
      ...
      case 'price-low': orderBy += 'price_cents ASC'; break;
      case 'price-high': orderBy += 'price_cents DESC'; break;
      case 'name': orderBy += 'title ASC'; break;
      case 'stock': orderBy += 'stock DESC, id DESC'; break;
      case 'newest':
    ...
    if (!q && !category && !priceRange) {
      try {
        ...
        logger.error('Error fetching news for homepage:', newsError);
        latestNews = [];
      }
    }
  ...
  try {
    ...
    `;
      ...
      products.forEach(p => {
        ...
        const stockBadge = p.stock > 0 ? `<span class="in-stock">✅ Còn hàng (${p.stock})</span>` : '<span class="out-of-stock">❌ Hết hàng</span>';
    ...
    res.json({ success: true, html: html, count: products.length });
  } catch (error) {
    logger.error('Error in filter API:', error);
...
router.get('/product/:slug', async (req, res) => {
  try {
    ...
    logger.error('Error in product route:', error);
